...
void GetIntersectons(Coordinates coords, Intersections& intersections)
{
	...
	std::set_intersection(temp[0].begin(),
		temp[0].end(),
		temp[1].begin(),
		temp[1].end(),
	...
	for (auto& i : intersections)
	{
		...
		if (it0 != coords[0].end())
		{
			temp0.emplace_back(std::distance(coords[0].begin(), it0) + 1);
			temp1.emplace_back(std::distance(coords[1].begin(), it1) + 1);
		}
	}
	...
	if (temp0.size() != temp1.size() || result.size() != intersections.size())
	{
		std::cout << "Something is wrong!!!\n";
	}
	std::sort(result.begin(), result.end());
	std::cout << "Shortest distance is: " << result[0] << ", res size: "<< result.size() << ", int size: " << intersections.size() << std::endl;
}
